...
func TestLoad(t *testing.T) {
	...
	if s.reg.getA(R08) != 0x12 {
		t.Error("Error in LDA zpgX with zero page overflow")
	}

	s.mem.Poke(0x1234, 0x67)
	s.executeLine([]uint8{0xAD, 0x34, 0x12})
	if s.reg.getA(R08) != 0x67 {
...
func TestStack(t *testing.T) {
	...
	s.reg.setSP(R08, 0xF0)
	s.reg.setA(R08, 0xA0)
	s.reg.setP(0x0A)
	s.executeLine([]uint8{0x48})
	if s.reg.getSP(R08) != 0xEF {
